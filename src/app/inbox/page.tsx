...
export default function SharedInboxPage() {
  ...
  // Inbound WhatsApp Simulation Modal State
  const [isSimulateOpen, setIsSimulateOpen] = useState(false);
  const [simPhone, setSimPhone] = useState('+91 98765 43210');
  const [simName, setSimName] = useState('Arihant Jain');
  const [simMessage, setSimMessage] = useState('Hi! I am interested in your product details.');
  ...
  return (
    <div className="-m-6 flex h-[calc(100vh-4rem)] w-[calc(100%+3rem)] overflow-hidden bg-slate-100">
        ...
        <div className="border-b border-slate-200 p-4 space-y-3">
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="flex items-center gap-1 rounded-xl bg-emerald-600 px-2.5 py-1 text-[11px] font-bold text-white hover:bg-emerald-700 shadow-2xs"
              title="Test Incoming WhatsApp Message"
            >
      ...
      {/* Simulate Inbound WhatsApp Message Modal */}
      {isSimulateOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-xs p-4">
          <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl space-y-4">
            ...
            <p className="text-xs text-slate-500">
              Simulate an incoming WhatsApp customer message from any mobile number to test live inbox routing and notifications.
            </p>
            ...
            <form onSubmit={handleSimulateInbound} className="space-y-3">
              ...
              <div>
                ...
                <input
                  ...
                  className="mt-1 w-full rounded-xl border border-slate-300 bg-white p-2.5 text-xs font-mono text-slate-900 focus:border-emerald-500 focus:outline-none"
                />
              </div>

              <div>
                <label className="text-xs font-semibold text-slate-700">Incoming Message Content</label>
                <textarea
                  rows={3}
                  required
                  placeholder="e.g. Hi! I need details on Diwali offer discount..."
                  value={simMessage}
              ...
              <div className="pt-3 flex justify-end gap-2">
                ...
                >
                  {simulating ? 'Sending Inbound...' : 'Send Test Inbound Message'}
                </button>
              </div>
            </form>
          </div>
